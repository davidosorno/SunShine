...
fun AppCompatTextView.currentDate(date: Date?){
    text = date?.let {
        setFormatDate(it, "EEE, MMM dd yyyy")
    } ?: run {
        ""
    }
}

@BindingAdapter("setCurrentLongDate")
fun AppCompatTextView.currentLongDate(longDate: Long?){
    text = longDate?.let {
        val date = Date(it*1000)
        setFormatDate(date, "EEE, MMM dd yyyy")
    } ?: run {
        ""
    }
}
...
@BindingAdapter("setHourFromLong")
fun AppCompatTextView.hourFromLong(date: Long){
    val formatDate = Date(date*1000) //I must add three 0 at the end of the number to get the right hour.
    text = setFormatDate(formatDate, "hh a")
}
...
fun AppCompatTextView.stringValue(stringValue: String?){
    text = stringValue?.let {
        resources.getString(R.string.weatherDescription, stringValue.capitalize(Locale.US))
    } ?: run {
        ""
    }
}
...
fun AppCompatTextView.temp(temp: Float?){
    text = temp?.let {
        resources.getString(R.string.value_temp, temp.toInt())
    } ?: run {
        ""
    }
}
...
fun AppCompatTextView.temp(temp: Int?){
    text = temp?.let {
        resources.getString(R.string.value_temp, temp)
    } ?: run {
        ""
    }
}
...
fun AppCompatTextView.humidity(humidity: Int?){
    text = humidity?.let {
        resources.getString(R.string.humidity_value, humidity)
    } ?: run {
        ""
    }
}
...
fun AppCompatTextView.pressure(pressure: Float?){
    text = pressure?.let {
        resources.getString(R.string.pressure_value, pressure)
    } ?: run {
        ""
    }
}
...
fun AppCompatTextView.wind(windSpeed: Int?){
    text = windSpeed?.let {
        resources.getString(R.string.value_temp, windSpeed)
    } ?: run {
        ""
    }
}
